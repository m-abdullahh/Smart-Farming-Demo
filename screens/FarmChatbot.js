...
const FarmChatbot = () => {
  const [messages, setMessages] = useState([
    {
      ...
      sender: "ChatBot",
      content:
        "Hi! You can consult me about Farm Management. How can I help you? 🚜",
  ...
  const predefinedQuestions = [
    "How can I manage my farm efficiently?",
  ...
  const handleSend = async (messageText) => {
    ...
    try {
      const response = await fetch(
        `https://m-abdu11ah-cropapp.hf.space/farm-management-chatbot?query='${encodeURIComponent(
          messageText
        )}'`,
        {
          method: "GET",
